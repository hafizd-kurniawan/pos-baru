...
-- Add category field to spare_parts table and update vehicle repair cost management
-- Migration: 002_add_spare_part_category_and_repair_cost_management

-- Add category column to spare_parts table
ALTER TABLE spare_parts 
ADD COLUMN category VARCHAR(50) NOT NULL DEFAULT 'General';

-- Update existing spare parts with default categories based on common automotive parts
UPDATE spare_parts SET category = 'Engine' WHERE name ILIKE '%engine%' OR name ILIKE '%oil%' OR name ILIKE '%filter%';
UPDATE spare_parts SET category = 'Brake' WHERE name ILIKE '%brake%' OR name ILIKE '%pad%' OR name ILIKE '%disc%';
UPDATE spare_parts SET category = 'Electrical' WHERE name ILIKE '%battery%' OR name ILIKE '%spark%' OR name ILIKE '%wire%' OR name ILIKE '%light%';
UPDATE spare_parts SET category = 'Transmission' WHERE name ILIKE '%transmission%' OR name ILIKE '%gear%' OR name ILIKE '%clutch%';
UPDATE spare_parts SET category = 'Suspension' WHERE name ILIKE '%shock%' OR name ILIKE '%spring%' OR name ILIKE '%strut%';
UPDATE spare_parts SET category = 'Tire' WHERE name ILIKE '%tire%' OR name ILIKE '%wheel%' OR name ILIKE '%rim%';
UPDATE spare_parts SET category = 'Body' WHERE name ILIKE '%body%' OR name ILIKE '%door%' OR name ILIKE '%panel%' OR name ILIKE '%bumper%';

-- Create index on category for better performance
CREATE INDEX idx_spare_parts_category ON spare_parts(category);

-- Create a function to automatically update vehicle repair cost when repair spare parts are added/updated
CREATE OR REPLACE FUNCTION update_vehicle_repair_cost()
RETURNS TRIGGER AS $$
DECLARE
    vehicle_id_var INTEGER;
    total_cost DECIMAL(12,2);
BEGIN
    -- Get vehicle_id from repair_orders table
    SELECT vehicle_id INTO vehicle_id_var
    FROM repair_orders 
    WHERE id = COALESCE(NEW.repair_order_id, OLD.repair_order_id);
    
    -- Calculate total cost of spare parts for this repair order
    SELECT COALESCE(SUM(rsp.quantity_used * sp.selling_price), 0) INTO total_cost
    FROM repair_spare_parts rsp
    JOIN spare_parts sp ON rsp.spare_part_id = sp.id
    WHERE rsp.repair_order_id = COALESCE(NEW.repair_order_id, OLD.repair_order_id);
    
    -- Update vehicle repair_cost
    UPDATE vehicles 
    SET repair_cost = total_cost,
        updated_at = CURRENT_TIMESTAMP
    WHERE id = vehicle_id_var;
    
    -- Also update the repair order actual_cost
    UPDATE repair_orders 
    SET actual_cost = total_cost,
        updated_at = CURRENT_TIMESTAMP
    WHERE id = COALESCE(NEW.repair_order_id, OLD.repair_order_id);
...
-- Create triggers to automatically update repair costs
DROP TRIGGER IF EXISTS trigger_update_repair_cost_on_insert ON repair_spare_parts;
DROP TRIGGER IF EXISTS trigger_update_repair_cost_on_update ON repair_spare_parts;
DROP TRIGGER IF EXISTS trigger_update_repair_cost_on_delete ON repair_spare_parts;

CREATE TRIGGER trigger_update_repair_cost_on_insert
    AFTER INSERT ON repair_spare_parts
    FOR EACH ROW
    EXECUTE FUNCTION update_vehicle_repair_cost();

CREATE TRIGGER trigger_update_repair_cost_on_update
    AFTER UPDATE ON repair_spare_parts
    FOR EACH ROW
    EXECUTE FUNCTION update_vehicle_repair_cost();

CREATE TRIGGER trigger_update_repair_cost_on_delete
    AFTER DELETE ON repair_spare_parts
    FOR EACH ROW
    EXECUTE FUNCTION update_vehicle_repair_cost();

-- Add some sample categories for demonstration
INSERT INTO spare_parts (code, name, description, category, unit, purchase_price, selling_price, stock_quantity, minimum_stock, is_active) 
VALUES 
    ('ENG001', 'Engine Oil 5W-30', 'Premium synthetic engine oil', 'Engine', 'liter', 45000, 65000, 50, 10, true),
    ('BRK001', 'Brake Pad Set', 'Ceramic brake pads for front wheels', 'Brake', 'set', 350000, 450000, 25, 5, true),
    ('ELC001', 'Car Battery 12V', 'Maintenance-free car battery', 'Electrical', 'pcs', 800000, 1200000, 15, 3, true),
    ('TIR001', 'Tire 185/65R15', 'All-season passenger tire', 'Tire', 'pcs', 600000, 850000, 20, 4, true),
    ('FIL001', 'Air Filter', 'High-performance air filter', 'Engine', 'pcs', 85000, 125000, 30, 8, true),
    ('SUS001', 'Shock Absorber', 'Heavy-duty shock absorber', 'Suspension', 'pcs', 450000, 650000, 12, 3, true),
    ('BOD001', 'Side Mirror', 'Electric side mirror with heating', 'Body', 'pcs', 280000, 420000, 8, 2, true),
    ('TRA001', 'Transmission Fluid', 'Automatic transmission fluid', 'Transmission', 'liter', 120000, 180000, 25, 5, true)
ON CONFLICT (code) DO NOTHING;

-- Insert categories as reference data (for frontend filtering)
...
INSERT INTO spare_part_categories (name, description) VALUES
    ('Engine', 'Engine related parts and fluids'),
    ('Brake', 'Braking system components'),
    ('Electrical', 'Electrical system and electronic components'),
    ('Transmission', 'Transmission and drivetrain parts'),
    ('Suspension', 'Suspension and steering components'),
    ('Tire', 'Tires and wheel related parts'),
